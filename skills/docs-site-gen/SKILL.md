...
9. **Visual Diagrams (Server-Safe SVG)** — When a section needs a visual (multi-component / multi-step / branching / time-series), use one of 5 archetypes from `references/templates.md` Diagram Templates. Match brand palette; pull mock data from Phase 2B evidence; centralize in `_components/Diagrams.tsx`. See `references/generation-rules.md` §4.9.
